...
from met_museum import MetMuseum, AsyncMetMuseum, DefaultAioHttpClient
from met_museum._utils import is_dict
...
logging.getLogger("met_museum").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[MetMuseum]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with MetMuseum(base_url=base_url, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncMetMuseum]:
    ...
    async with AsyncMetMuseum(base_url=base_url, _strict_response_validation=strict, http_client=http_client) as client:
        yield client
